...
export function describeInstallationFile(change: FileChange): FileChange & { ownership: string; purpose: string } {
  ...
  const detail = change.merged ? ["Merged with your edits", "Crewbie refreshed only its managed block; everything you wrote outside it is kept."]
    ...
    : path.startsWith(".github/agents/crewbie-") ? ["Crewbie-managed", "The specialist charter links shared rules and scoped memory; edit freely outside the managed block."]
...
export function renderInstallationPreview(changes: FileChange[], labels: readonly string[], repository: string, kept: readonly string[] = []): string {
  return [
    ...
    ...(kept.length ? [`KEPT AS YOU EDITED | ${kept.length} files`, ...kept.map((path) => `  ${path}`), ""] : []),
